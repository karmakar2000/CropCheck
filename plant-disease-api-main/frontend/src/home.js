...
export const ImageUpload = () => {
  ...
  const sendFile = async () => {
    if (image) {
      ...
      let res = await axios({
        method: "post",
        url: "http://44.209.119.96/predict",
        data: formData,
      });
  ...
  const parseText = (text) => {
    ...
    // Replace matched text with corresponding HTML tags
  ...
  return (
    <React.Fragment>
      ...
      {
        data && (
          <div className={classes.solution_container}>
            <div
              className="solution_inner"
              dangerouslySetInnerHTML={{ __html: parseText(data.solution ?? '') }}
            ></div>
          </div>
        )
      }
